...
struct feature*** nbrs, int max_nn_chks) 
{
	...
	struct bbf_data* bbf_data_a; //bbf_data是一个用来存放临时特征数据和特征间距离的缓存结构
	...
	while (min_pq->n > 0 && t < max_nn_chks)
	{
		...
		for (i = 0; i < expl->n; i++)
		{
			...
			bbf_data_a = (bbf_data*)malloc(sizeof(struct bbf_data));//新建bbf结构
			if (!bbf_data_a)
			{   //出错处理
				fprintf(stderr, "Warning: unable to allocate memory," " %s line %d\n", __FILE__, __LINE__);
				goto fail;
			}
			bbf_data_a -> old_data = tree_feat->feature_data;//保存第i个特征点的feature_data域以前的值
			bbf_data_a -> d = descr_dist_sq(feat, tree_feat);//当前搜索点和目标点之间的欧氏距离
			tree_feat->feature_data = bbf_data_a;//将bbf结构赋给此特征点的feature_data域
	...
	for (i = 0; i < n; i++)
	{
		bbf_data_a = (bbf_data*) _nbrs[i]->feature_data;
		_nbrs[i]->feature_data = bbf_data_a->old_data;//将之前的数据赋值给feature_data域
		free(bbf_data_a);
	}
	*nbrs = _nbrs;
	return n;
...
fail:
	...
	{
		bbf_data_a = (bbf_data*)_nbrs[i]->feature_data;
		_nbrs[i]->feature_data = bbf_data_a->old_data;
		free(bbf_data_a);
	}
	free(_nbrs);
	*nbrs = NULL;
...
{
	struct kd_node* kd_node_a;

	kd_node_a = (kd_node*)malloc(sizeof(struct kd_node));//分配内存
	memset(kd_node_a, 0, sizeof(struct kd_node));
	kd_node_a->ki = -1;//枢轴索引
	kd_node_a->features = features;//节点对应的特征点集
	kd_node_a->n = n;//特征点的个数
